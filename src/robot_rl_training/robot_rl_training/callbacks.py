...
class TrainingMonitorCallback(BaseCallback):
    """Adds task-level monitoring on top of SB3's built-in PPO metrics.

    PPO losses and optimizer metrics tell you whether the learner is numerically stable.
    Reach/grasp/lift/hold rates tell you whether the robot is actually learning the task.
    """
...
class PeriodicEvalCallback(BaseCallback):
    """Runs evaluation in a separate Isaac Python process.

    This keeps evaluation isolated from the training environment state while preserving the
    design goal that Isaac Sim owns the entire simulator loop natively.
    """
    ...
    def _on_step(self) -> bool:
        ...
        print(
            ...
            f"stable_hold={summary['stable_hold_rate']:.2%}"
        )
        return True
