...
class HumanoidBrickPickEnv(gym.Env):
    ...
    def _load_isaac_modules(self) -> None:
        ...
        self._ArticulationController = ArticulationController
    ...
    def _build_scene(self) -> None:
        # Each environment owns its own stage, articulation, and objects locally in Isaac Sim.
        # This keeps PPO simulation completely inside the Isaac Python process and avoids any
        # ROS 2 pub/sub or service traffic in the training loop.
        ...
        self.table = self.world.scene.add(
            self._FixedCuboid(
                ...
                name="training_table",
                position=np.array([0.40, 0.0, 0.40], dtype=np.float32),
                scale=np.array([0.60, 0.80, 0.04], dtype=np.float32),
                color=np.array([0.45, 0.32, 0.18], dtype=np.float32),
            )
        )
        self.table_height = 0.42

        result, imported_prim_path = self._omni_kit_commands.execute(
        ...
        if not result:
            raise RuntimeError(f"Failed to import URDF: {self.training_config.runtime_urdf_path}")
        self.robot_prim_path = imported_prim_path

        self.brick = self.world.scene.add(
            self._DynamicCuboid(
                prim_path="/World/toy_brick",
                name="toy_brick",
                position=np.array([0.38, 0.0, self.table_height + 0.01], dtype=np.float32),
        ...
        self.controller = self._ArticulationController()

        self.world.reset()
        self.articulation.initialize()
        self.controller.initialize(self.articulation)
        ...
        self.ee_prim = self._get_prim_at_path(
            f"{self.robot_prim_path}/{self.training_config.end_effector_parent_link}"
        )
    ...
    def _set_robot_home(self) -> None:
        ...
        self.controller.apply_action(self._ArticulationAction(joint_positions=joint_positions))
    ...
    def _get_brick_position(self) -> np.ndarray:
        return np.array(self.brick.get_world_pose()[0], dtype=np.float32)
    ...
    def _get_observation(self) -> dict[str, np.ndarray | bool]:
        ...
        brick_position = self._get_brick_position()
        ee_position = self._get_end_effector_position()
        horizontal_distance = np.linalg.norm(brick_position[:2] - ee_position[:2])
        grasp_success = bool(horizontal_distance < 0.04 and np.mean(gripper_positions) > 0.02 and brick_position[2] > self.table_height + 0.01)
        return {
    ...
    def step(self, action: np.ndarray):
        ...
        self.controller.apply_action(self._ArticulationAction(joint_positions=joint_targets))
    ...
    def close(self) -> None:
        # The shared SimulationApp is intentionally not closed here because the trainer
        # owns the Isaac runtime for the whole process.
        return None
