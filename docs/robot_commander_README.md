...
# Robot Commander
```xml
ros2 launch robot_commander robot_moveit.launch.xml use_simulation:=false//启动robot_moveit实机模式
ros2 launch robot_commander robot_moveit.launch.xml //启动robot_moveit非实机仿真模式
<!-- ros2 run robot_commander commander //启动订阅的自定义的相关话题 -->
```
